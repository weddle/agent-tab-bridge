...
export interface HubState { version: typeof HUB_STATE_VERSION; machines: HubMachineRecord[]; endpoints: HubEndpointRecord[]; connectors: string[]; }
...
function validState(value: unknown): value is HubState {
  ...
  if (state.version !== HUB_STATE_VERSION || !Array.isArray(state.machines) || !Array.isArray(state.endpoints) || !Array.isArray(state.connectors)) return false;
...
export class HubStateStore {
  ...
  async load(): Promise<HubState> {
    ...
    if (value === undefined) this.state = { version: HUB_STATE_VERSION, machines: [], endpoints: [], connectors: [] };
    else if (validState(value)) this.state = clone(value);
    else throw new Error("invalid hub state");
    return clone(this.state);
  }
  async save(state: HubState): Promise<void> { if (!validState(state)) throw new TypeError("invalid hub state"); await atomicWritePrivateJson(this.filePath, state); this.state = clone(state); }
  async update(mutator: (state: HubState) => HubState | Promise<HubState>): Promise<HubState> { return this.saveAndReturn(await mutator(await this.load())); }
  private async saveAndReturn(state: HubState): Promise<HubState> { await this.save(state); return clone(state); }
}
...
export class HubService {
  ...
  async enrollMachine(pairing: PinnedPeerKeyset, alias = pairing.pinnedPeerKey.principalId): Promise<HubMachineRecord> {
    ...
    const peer = pairing.pinnedPeerKey;
    ...
    const record: HubMachineRecord = { alias, principalId: peer.principalId, publicKeySpki: peer.publicKeySpki, fingerprint: peer.fingerprint, pairing: clone(pairing), presence: "offline", lastSeen: null };
  ...
  async forgetMachine(principalId: string): Promise<void> {
    for (const client of this.clients) if (client.machineId === principalId) client.socket.destroy();
    await this.stateStore.update((state) => ({ ...state, machines: state.machines.filter((machine) => machine.principalId !== principalId), endpoints: state.endpoints.filter((endpoint) => endpoint.machineId !== principalId) }));
    this.stateCache = await this.stateStore.load();
  }
  ...
  private sendControl(client: HubClient, value: unknown): void { client.socket.write(encodeHubFrame(Buffer.from(JSON.stringify(value), "utf8"), this.maxFrameBytes)); }
  private async handleFrame(client: HubClient, frame: Buffer, peerFingerprintValue: string | undefined): Promise<void> {
    ...
    if (!client.machineId) {
      if (type === "pairInvitationRequest" && this.pairing) {
        this.sendControl(client, { type: "pairInvitation", code: this.pairing.code, invitation: this.pairing.invitation });
        return;
      }
      if (type === "pairHello" && this.pairing && message.hello && typeof message.hello === "object") {
        ...
        const response = await this.respondPairing(hello, hello.machine.fingerprint);
    ...
    for (const target of this.clients) if (target !== client && target.machineId) target.socket.write(encodeHubFrame(frame, this.maxFrameBytes));
  }
  private async accept(socket: TLSSocket): Promise<void> {
    ...
    socket.on("close", () => { this.clients.delete(client); if (client.machineId) void this.setPresence(client.machineId, "offline"); });
    socket.on("error", () => socket.destroy());
  }
}

export function clientTlsOptions(pairing: PinnedPeerKeyset, identity: StoredIdentity, hubCertificatePem: string): TlsPinningOptions & { ca: string; key: string; cert: string } {
  const certificate = selfSignedCertificate(identity, "atb machine");
  return { ...pinnedTlsOptions(pairing), ca: hubCertificatePem, key: certificate.keyPem, cert: certificate.certPem };
}
