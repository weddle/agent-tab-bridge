...
const session = (overrides: Partial<SessionRecord> = {}): SessionRecord => ({ id: "task-1", controllerPrincipalId: "sha256/controller", displayControllerName: "CLI", taskLabel: "research", requestedCapabilities: ["cdp"], createdAt: 10, expiresAt: 10 + 60_000, state: "pending", ...overrides });
...
describe("native protocol validation", () => {
  ...
  it("requires version, role, and complete handshake transcript fields", () => {
    ...
    expect(validateNativeMessage({ ...hello, version: 2 })).toBe(false);
