...
export const DEFAULT_TASK_SESSION_TTL_MS = 30 * 60_000;
export const MAX_REVOKED_TASK_SESSION_TOMBSTONES = 64;

export const MAX_TASK_SESSION_TTL_MS = 24 * 60 * 60_000;
export type TaskSession = Readonly<{ id: string; controllerPrincipalId: string; displayControllerName: string; taskLabel: string; requestedCapabilities: readonly TaskSessionCapability[]; createdAt: number; expiresAt: number; state: TaskSessionState }>;
...
export class TaskSessionManager {
  ...
  open(input: OpenTaskSessionInput): TaskSession {
    ...
    const ttl = input.ttlMs ?? DEFAULT_TASK_SESSION_TTL_MS; if (!Number.isSafeInteger(ttl) || ttl <= 0 || ttl > MAX_TASK_SESSION_TTL_MS) throw new TaskSessionError("invalidTtl", "invalid session TTL");
    const now = this.now(); const session: TaskSession = { id: this.ids(), controllerPrincipalId: display(input.controllerPrincipalId, "controller principal ID"), displayControllerName: display(input.controllerName, "controller name"), taskLabel: display(input.taskLabel, "task label"), requestedCapabilities: capabilities as TaskSessionCapability[], createdAt: now, expiresAt: now + ttl, state: "pending" };
    ...
    stored.timer = this.setTimer(() => { void this.revoke(session.id, "ttlExpired").catch(() => {}); }, ttl); stored.timer.unref?.(); this.stored.set(session.id, stored); this.emit({ type: "pending", session: clone(session) }); return clone(session);
  }
  ...
  async sweepExpired(): Promise<void> { const now = this.now(); await Promise.all([...this.stored.values()].filter(({ session }) => session.state !== "revoked" && session.expiresAt <= now).map(async ({ session }) => await this.revoke(session.id, "ttlExpired"))); }
