...
export async function createCompanionBrokerClient(options: { directory?: string } = {}) {
  ...
  return createBrokerClient({ socketPath: broker.defaultBrokerSocketPath({ directory: options.directory }), token: state.brokerSecret });
}
import net from "node:net";
import type { BrokerEvent } from "../atb.js";

export type BrokerClientOptions = { socketPath: string; token: string; socketFactory?: () => net.Socket };
